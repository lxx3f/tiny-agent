...
class VectorStore:
    ...
    def __init__(self, documents: List[str] = None) -> None:
        ...
        :param documents: 文档列表，默认为空。
        """
        if documents is None:
            documents = []
        self.documents = documents  # 存储文档内容
        self.vectors = []  # 存储文档的向量表示
        self.doc_ids = []  # 存储文档的唯一ID
        self.vector_ids = []  # 存储向量块的唯一ID

        # 为每个文档生成唯一ID
        self.doc_ids = [str(uuid.uuid4()) for _ in self.documents]
    ...
    def get_vector(self, EmbeddingModel) -> List[Dict[str, List[float]]]:
        ...
        # 为每个文档生成向量并生成唯一向量块ID
        self.vectors = [
            EmbeddingModel.get_embedding(doc) for doc in self.documents
        ]
    ...
    def persist(self, path: str = RAG_DEFAULT_SAVE_PATH):
        """
        ...
        # 将文档内容和文档ID存储到文本文件中
        with open(os.path.join(path, 'documents.txt'), 'w',
                  encoding='utf-8') as f:
            for doc, doc_id in zip(self.documents, self.doc_ids):
                f.write(f"{doc_id}\t{doc}\n")
        # 将向量ID存储到文本文件中
        with open(os.path.join(path, 'vector_ids.txt'), 'w',
                  encoding='utf-8') as f:
            for vector_id in self.vector_ids:
                f.write(f"{vector_id}\n")

    def load_vector(self, path: str = RAG_DEFAULT_SAVE_PATH):
        """
        ...
        # 加载文档内容和文档ID
        with open(os.path.join(path, 'documents.txt'), 'r',
                  encoding='utf-8') as f:
            self.documents = []
            self.doc_ids = []
        ...
        # 加载向量ID
        with open(os.path.join(path, 'vector_ids.txt'), 'r',
                  encoding='utf-8') as f:
            self.vector_ids = [line.strip() for line in f.readlines()]

    def get_similarity(self, vector1: List[float],
    ...
    def print_info(self):
        ...
        for i, (doc_id, doc, vector_id, vector) in enumerate(
                zip(self.doc_ids, self.documents, self.vector_ids,
                    self.vectors)):
            print(f"文档 {i+1}:")
            print(f"  文档ID: {doc_id}")
            print(f"  文档内容: {doc}")
